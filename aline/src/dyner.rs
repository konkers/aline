// Imported from dyner crate:
//     https://github.com/nikomatsakis/dyner
...
impl<T> core::ops::Deref for RefMut<T> {
    type Target = T;
...
impl<T> core::ops::DerefMut for RefMut<T> {
...
impl<'me, Output> Drop for InlineFuture<'me, Output> {
    fn drop(&mut self) {
        unsafe {
            core::ptr::drop_in_place(self.future);
        }
    }
}
...
pub struct InlineRefCellFuture<'me, Output> {
    future: core::cell::RefMut<'me, dyn Future<Output = Output> + 'me>,
}
...
impl<'me, Output> InlineRefCellFuture<'me, Output> {
    ...
    /// * `*future` must not be used again without having been reinitialized (which must occur after `'me` ends)
    pub unsafe fn new(
        future: core::cell::RefMut<'me, MaybeUninit<impl Future<Output = Output>>>,
    ) -> Self {
        let future = core::cell::RefMut::map(
            future,
...
impl<'me, Output> Drop for InlineRefCellFuture<'me, Output> {
    fn drop(&mut self) {
        unsafe {
            core::ptr::drop_in_place(&mut *self.future);
        }
    }
}
